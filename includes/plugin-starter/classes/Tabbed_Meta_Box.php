...
class Tabbed_Meta_Box {
	...
	function meta_box( $object, $box ) {
		...
		$meta_box_options = $this->meta_box_args( $object->post_type , $object ); ?>
		...
		<p>
			...
			<select name="<?php echo $name; ?>" id="<?php echo $name; ?>" >
				...
				<?php $i = 0; foreach ( $args['options'] as $option => $val ) { $i++; ?>
					<option rel="<?php echo esc_attr( $value ); ?>" value="<?php echo esc_attr( $val ); ?>" <?php selected( esc_attr( $val ), esc_attr( $value ) ); ?>><?php echo ( !empty( $args['use_key_and_value'] ) ? $option : $val ); ?></option>
				<?php } ?>
			</select>
		</p>
		<?php
	...
	public function meta_box_radio( $args = array(), $value = false ) {
		...
		<p>
			...
			<?php foreach ( $args['options'] as $option => $val ) { ?>
				...
				<label for="<?php echo $name; ?>"><?php echo ( !empty( $args['use_key_and_value'] ) ? $option : $val ); ?></label>
			<?php } ?>
		</p>
		<?php
	}
	...
	function save_meta_box( $post_id, $post ) {
		...
		foreach ( $metadata as $name => $meta ) {
			...
			if ( isset( $new_meta_value ) && '' == $meta_value )
